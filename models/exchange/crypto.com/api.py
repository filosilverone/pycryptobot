"""Remotely control your Crypto.com account via their API : https://Crypto.com-docs.github.io/apidocs/spot/en"""
...
class AuthAPI(AuthAPIBase):
    def __init__(
        ...
        api_key: str = "",
        api_secret: str = "",
        #api_url: str = "https://api.crypto.com/v2/",
		api_url: str = "https://uat-api.3ona.co/v2/",
        order_history: list = [],
        recv_window: int = 5000,
    ) -> None:
        """Crypto.com API object model

        Parameters
        ----------
        api_key : str
            Your Crypto.com account portfolio API key
        api_secret : str
            Your Crypto.com account portfolio API secret
        api_url
            Crypto.com API URL
        """
        ...
        # validate Crypto.com API
        if api_url not in valid_urls:
            raise ValueError("Crypto.com API URL is invalid")

        # validates the api key is syntactically correct
        p = re.compile(r"^[A-z0-9]{64,64}$")
        if not p.match(api_key):
            self.handle_init_error("Crypto.com API key is invalid")

        # validates the api secret is syntactically correct
        p = re.compile(r"^[A-z0-9]{64,64}$")
        if not p.match(api_secret):
            self.handle_init_error("Crypto.com API secret is invalid")
    ...
    def getAccounts(self) -> pd.DataFrame:
        ...
        try:
            resp = self.authAPI(
                "GET", "/api/v2/account", {"recvWindow": self.recv_window}
            )
    ...
    def getAccount(self) -> pd.DataFrame:
        """Retrieves all accounts for Crypto.com as there is no specific account id"""
    ...
    def getOrders(
        ...
        if market != "":
            ...
            if not self._isMarketValid(market):
                raise ValueError("Crypto.com market is invalid.")
        else:
    ...
    def getTradeFee(self, market: str) -> float:
        ...
        # Crypto.com US does not currently define "/sapi/v1/asset/tradeFee" in its API
        if self._api_url == "https://api.Crypto.com.us":
    ...
    def getTicker(self, market: str = DEFAULT_MARKET, websocket=None) -> tuple:
        ...
        # validates the market is syntactically correct
        if not self._isMarketValid(market):
            raise TypeError("Crypto.com market required.")
    ...
    def marketBuy(
        ...
        try:
            ...
            Logger.error(f"{ts} Crypto.com  marketBuy {str(err)}")
            return []
    ...
    def marketSell(
        ...
        try:
            ...
            ts = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
            Logger.error(f"{ts} Crypto.com  marketSell {str(err)}")
            return []
...
class PublicAPI(AuthAPIBase):
    def __init__(self, api_url="https://api.Crypto.com.com") -> None:
        """Crypto.com API object model
        ...
        valid_urls = [
            "https://api.Crypto.com.com",
            "https://api.Crypto.com.us",
            "https://testnet.Crypto.com.vision",
        ]
...
class WebSocket(AuthAPIBase):
    def __init__(
        ...
        api_url="https://api.Crypto.com.com",
        ws_url: str = "wss://stream.Crypto.com.com:9443",
    ) -> None:
        # options
        self.debug = False
        ...
        valid_ws_urls = [
            "wss://stream.Crypto.com.com:9443",
            "wss://stream.Crypto.com.com:9443/",
        ]

        # validate Crypto.com Websocket URL
        if ws_url not in valid_ws_urls:
            raise ValueError("Crypto.com WebSocket URL is invalid")
...
class WebSocketClient(WebSocket, AuthAPIBase):
    def __init__(
        ...
        if api_url == "https://api.Crypto.com.us":
            valid_ws_urls = [
                "wss://stream.Crypto.com.us:9443",
                "wss://stream.Crypto.com.us:9443/",
            ]
        else:
            valid_ws_urls = [
